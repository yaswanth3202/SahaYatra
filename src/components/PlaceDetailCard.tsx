...
export function PlaceDetailCard({ place, onClose }: PlaceDetailCardProps) {
  ...
  useEffect(() => {
    const loadPlaceImages = async () => {
      setLoading(true);
      try {
        console.log(`Loading images for: ${place.place_name}`);
        const unsplashImages = await ImageService.getPlaceImages(place.place_name, 5);
        
        if (unsplashImages.length > 0) {
          const imageUrls = unsplashImages.map(img => ImageService.getOptimizedImageUrl(img, 'full'));
          setImages(imageUrls);
          console.log(`Loaded ${imageUrls.length} real images for ${place.place_name}`);
        } else {
          // Fallback to generic images if no specific images found
          ...
          console.log(`Using fallback images for ${place.place_name}`);
        }
      } catch (error) {
            ...
            {loading ? (
              <div className="w-full h-full bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-blue-600 font-medium">Loading images for {place.place_name}...</p>
                </div>
              </div>
            ) : images.length > 0 ? (
              <img
                ...
                onError={(e) => {
                  ...
                  target.src = `https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&h=600&fit=crop&crop=center&q=80&auto=format&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D`;
                }}
              />
            ) : (
