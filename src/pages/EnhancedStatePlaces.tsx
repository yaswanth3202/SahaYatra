...
export function EnhancedStatePlaces() {
  ...
  useEffect(() => {
    if (stateCode && stateCode !== currentStateCode) {
      console.log(`Switching from ${currentStateCode} to state: ${stateCode}`);
      setIsLoadingState(true);
      setCurrentStateCode(stateCode);
      
      // Clear ALL previous state data immediately
      setPlaceImages(new Map());
      ...
      if (details) {
        ...
        // Filter places for the current state
        const currentStatePlaces = allPlaces.filter(place => place.stateCode === stateCode);
        setStatePlaces(currentStatePlaces);
        console.log(`Loaded ${currentStatePlaces.length} places for state ${stateCode}`);
  ...
  }, [stateCode, currentStateCode, allPlaces]);
  ...
  const loadPlaceImages = async (place: Place) => {
    ...
    if (placeImages.has(placeId) || loadingImages.has(placeId)) {
      return;
    }

    setLoadingImages(prev => new Set(prev).add(placeId));

    try {
      const images = await ImageService.getPlaceImages(place.place_name, 3);
      setPlaceImages(prev => new Map(prev).set(placeId, images));
    } catch (error) {
      console.error(`Error loading images for ${place.place_name}:`, error);
    } finally {
  ...
  const getPlaceImageUrl = (place: Place): string => {
    const placeImagesList = placeImages.get(place._id);
    if (placeImagesList && placeImagesList.length > 0) {
      return ImageService.getOptimizedImageUrl(placeImagesList[0], 'regular');
    }
    
    // Fallback to a generic India tourism image
    return `https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center&q=80&auto=format&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D`;
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {isLoadingState ? (
          <div className="text-center py-16">
            ...
            <p className="text-lg text-gray-600">Loading places for {stateCode}...</p>
          </div>
                ...
                <div className="relative h-56 overflow-hidden">
                  {loadingImages.has(place._id) ? (
                    ...
                    <img
                      ...
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        target.src = `https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center&q=80&auto=format&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D`;
                      }}
                    />
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
